...
wget -xi "$BASEDIR/profiles/esri_modules_3.5.txt" --base=http://serverapi.arcgisonline.com/jsapi/arcgis/3.5amd/js/
...
do
  perl -pi -e "
    ...
    s/layers\/layer/layers\/Layer/; # fix layer capitalization
    s/esri\/map/esri\/Map/; # another capitalization fix
    s/tasks\/locator/tasks\/Locator/; # another capitalization fix
    " $f
done
perl -pi -e "
    s/\,ar:1.*zh-cn\":1//;
